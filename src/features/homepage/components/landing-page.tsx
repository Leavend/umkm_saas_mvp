...
export const LandingPage = ({ content }: LandingPageProps) => {
  const { brandName, navLinks, actions, hero, features, howItWorks, testimonials, pricing, cta, footer } = content;

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-slate-100">
      <LandingNavigation brandName={brandName} navLinks={navLinks} actions={actions} />
...
const LandingNavigation = ({ brandName, navLinks, actions }: LandingNavigationProps) => (
  <nav className="sticky top-0 z-50 border-b border-slate-200/60 bg-slate-50/95 backdrop-blur supports-[backdrop-filter]:bg-slate-50/80">
    <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
      ...
      <div className="hidden items-center space-x-6 md:flex">
        {navLinks.map((link) => (
          <Link key={link.href} href={link.href} className="text-slate-600 transition-colors hover:text-blue-600">
            {link.label}
          </Link>
        ))}
      </div>
  ...
  <section className="relative overflow-hidden py-20 sm:py-32">
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-4xl text-center">
        ...
        <p className="mx-auto mb-10 max-w-2xl text-lg text-slate-600 sm:text-xl">{hero.description}</p>
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      ...
      <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
        {features.items.map((feature) => (
          <Card key={feature.key} className="group relative overflow-hidden border-slate-200 bg-white/70 backdrop-blur-sm transition-all hover:shadow-lg">
      ...
      <div className="grid gap-8 md:grid-cols-3">
        {howItWorks.steps.map((step) => (
          <div key={step.title} className="relative">
            ...
            <h3 className="mb-2 text-xl font-semibold text-slate-800">{step.title}</h3>
  ...
  <section id="testimonials" className="bg-white py-20 sm:py-32">
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div className="mx-auto mb-16 max-w-2xl text-center">
        ...
        <p className="mt-4 text-lg text-slate-600">{testimonials.description}</p>
      </div>

      <div className="grid gap-6 md:grid-cols-3">
        {testimonials.items.map((testimonial) => (
          <Card key={testimonial.name} className="relative border-slate-200 bg-white/70 backdrop-blur-sm">
            <CardContent className="p-6">
              ...
              <p className="mb-4 text-slate-600 italic">&ldquo;{testimonial.content}&rdquo;</p>
              <div>
        ...
        <Card className="relative overflow-hidden border-2 border-blue-300 bg-white/70 backdrop-blur-sm">
          ...
          <CardContent className="p-8">
            <div className="mb-8 text-center">
              <h3 className="text-2xl font-bold text-slate-800">{pricing.planName}</h3>
              <div className="mt-4 flex items-baseline justify-center">
                <span className="text-5xl font-bold text-slate-800">{pricing.price}</span>
                <span className="ml-2 text-slate-600">{pricing.priceSuffix}</span>
              </div>
            ...
            <p className="mt-4 text-center text-xs text-slate-500">{pricing.footnote}</p>
          </CardContent>
        </Card>
      </div>
    </div>
  ...
  <section className="bg-gradient-to-r from-blue-100/70 to-purple-100/70 py-20 sm:py-32">
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-2xl text-center">
        <h2 className="text-3xl font-bold tracking-tight text-slate-800 sm:text-4xl">{cta.heading}</h2>
        <p className="mt-4 text-lg text-slate-600">{cta.description}</p>
        <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:justify-center">
          <Link href="/dashboard">
            <Button size="lg" className="cursor-pointer gap-2 bg-gradient-to-r from-blue-500 to-purple-600 px-8 py-6 text-base hover:from-blue-600 hover:to-purple-700">
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      <div className="py-16">
        <div className="grid gap-8 md:grid-cols-4">
          ...
          <div>
            <h3 className="mb-4 font-semibold text-slate-800">{footer.product.title}</h3>
          ...
          <div>
            <h3 className="mb-4 font-semibold text-slate-800">{footer.support.title}</h3>
            <ul className="space-y-3 text-sm text-slate-600">
              <li>
        ...
        <div className="mt-16 border-t border-slate-200 pt-8 text-center text-sm text-slate-500">
          <p>&copy; 2025 {brandName}. {footer.copyright}</p>
        </div>
      </div>
    </div>
  </footer>
